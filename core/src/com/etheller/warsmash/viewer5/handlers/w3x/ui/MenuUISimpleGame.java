...
public class MenuUISimpleGame implements WarsmashMenuUI {
			...
			if (!this.beginGameInformation.loadingStarted) {
				...
				if (this.loadingMap != null) {
					...
					try {
						...
						final MapLoader mapLoader = this.loadingMap.viewer.createMapLoader(this.loadingMap.map, this.loadingMap.mapInfo, localPlayerIndex);
	...
	private void internalStartMap(final String mapFilename) {
		...
		try {
			...
			final int loadingScreen = mapInfo.getGameDataSet();
